...
Но с помощью безымянных функций можно не только представлять функции нескольких переменных как функцию одного переменного. Мы увидим, что рекурсию, арифметические действия и даже сами натуральные числа можно свести к одним лишь безымянным функциям.
...
Наиболее разумным представляется второй вариант, так как в таком случае смысл выражения $λx.\;x$ не зависит от того, частью какого выражения оно является. Однако, одного лишь разрешения неоднозначности в пользу того или иного варианта недостаточно, так как неоднозначность возникает в процессе вычисления выражений.
...
Вхождение переменной $\color{red}x$ в терм $\color{red}t$ называется *связанным,* если оно связано с одной из абстракций в $\color{red}t$. Вхождение переменной $\color{red} x$ связано с абстракцией $λ{\color{red}x}.\; {\color{red}u}$ в том случае, когда $\color{red}x$ входит в $\color{red}u$ и это вхождение не является частью иного терма $λ{\color{red}x}.\; {\color{red}v}$, входящего в $\color{red}u$.
...
Два терма называются *альфа-эквивалентными,* если один из них можно преобразовать в другой переименованием переменных. К примеру, выражение $λx.\;λx.\; x$ альфа-эквивалентно выражению $λa.\; λb.\; b$.
...
Выражение, соответствующее левой части этого правила, называется *редексом*[^redex]. Вычисление составного выражения сводится к замене одного из входящих в него редексов на соответствующее ему выражение в правой части правила.
...
Пусть некоторая функция применяется к натуральному числу. Она может либо вычислиться в некоторое значение $v$, если число является нулём, либо вычислиться в некоторое другое значение, если число следует за некоторым другим числом $n$. Это другое значение, вообще говоря, зависит от $n$. Предположим, что значение может быть вычисленно применением функции $f$ к $n$.
...
Отсюда мы немедленно получаем требуемую функцию $Y$:
