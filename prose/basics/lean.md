...
Переходим по ссылкам и скачиваем оба приложения, после чего устанавливаем сначала Git, а затем Visual Studio Code. Открываем Visual Studio Code. На панели слева щёлкаем по иконке с четырмя квадратами — откроется панель расширений. Вводим `lean4` в строку поиска и устанавливаем расширение.

Нам осталось лишь открыть проект. Используем Git, чтобы скачать и открыть заготовленный проект. Для этого нажимаем клавишу F1 и в появившейся строке поиска вводим `git clone` и нажимаем `Enter`. Visual Studio Code попросит URL репозитория, вставляем в поле ввода `https://github.com/suhr/polygon.git` и снова нажимаем `Enter`. Выбираем директорию, в которую будет скачена директория с проектом.
...
- Запись `let x := v;  e` означает выражение `e`, в котором `x` определено как `v`. Точку с запятой можно опусить, если `e` начинается с новой строки
...
Ввод символа начинается с ввода обратной косой черты, после которой следует имя символа. Если введёное имя однозначно определяет требуемый символ и не является префиксом имени какого-то другого символа, метод ввода немедленно заменяет введёное имя на требуемый символ. В противном случае, ввод символа можно завершить, нажав клавишу `Tab` или пробел.
...
Lean это *интерактивное* средство докзательства теорем. Панель Lean Infoview в правой половине экрана показывает различную полезную информацию. Одной из вещей, которую показывает эта панель, является контекст и ожидаемый тип в том месте, куда установлен курсор редактора.
...
Тип `Prod α β` представляет тип пар значений, где первое значение в паре имеет тип `α`, а второе значение имеет тип `β`. Тип пар также называют произведение типов `α` и `β` и обозначают как `α × β` (cимвол `×` вводится как `\times`).
...
Результатом применения `Prod.fst` и `Prod.snd` к значению типа `Prod α β` явлется, соответственно, левое и правое значение в паре. Например, результатом применения `Prod.fst` к `Prod.mk 3 5` является число `3`, когда же результатом применения `Prod.snd` является число `5`.
...
Рассмотрим теперь процесс доказательства высказваний на примерах.
...
У нас есть свидетельство истинности $P ∧ (Q ∨ R)$, и естественнно разобрать его на свидетельство $P$ и свидетельство $Q ∨ R$. Для этого можно использовать функции `And.left` и `And.right`. Но `And` это индуктивный тип с единственным конструктором, что позволяет разобрать значение с помощью сопоставления с образцом:
...
Значение типа `(P ∧ Q) ∨ (P ∧ R)` можно пострить либо из значения `P ∧ Q`, либо из значения `P ∧ R`. Выбираем первое, применив `Or.inl`:
...
Если значения взаимозаменяемы, то любое свойство, что верно для одного значения, верно и для другого. Функция `Eq.subst` утвержает в точности это:
...
В качестве таких значений возмём значения `false` и `true` типа `Bool`. Этот индуктивный тип определён в Lean следующим образом:
